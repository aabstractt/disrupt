...
func (kt *KoTHTick) Tick() {
	...
	if t == nil {
		common.Log.Panic("KoTH team not found")
	...
	for wName := range t.Tracker().Cuboids() {
		...
		if w == nil {
			common.Log.WithField("world", wName).Error("KoTH world not found")

			continue
		}

		if kt.capturingBy != "" {
			if p, ok := common.SRV.PlayerByXUID(kt.capturingBy); ok && p.World() == w && kt.bbox.Vec3Within(p.Position()) {
				return
			}
		}
